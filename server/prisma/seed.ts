...
async function main() {
  ...
  const SafeTechDataSet = await readRiskCSV<SafeTech>(
    "SafeTech.csv",
    safeTechMapper
  );
  const StateDataSet = await readRiskCSV<State>("State.csv", stateMapper);
  IsoDataSet.forEach(async (element) => {
    await prisma.iSO.create({
      data: element,
    });
  });

  StateDataSet.forEach(async (element) => {
    await prisma.state.create({
      data: element,
    });
  });

  SafeTechDataSet.forEach(async (element) => {
