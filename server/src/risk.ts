...
export const NyCrane: Hueristic = (
  iso: ISO,
  state: State,
  _safeTech: SafeTech
): boolean => {
...
export const CoRailroad: Hueristic = (
  iso: ISO,
  state: State,
  _safeTech: SafeTech
): boolean => {
...
export const riskTier = (
  ...
  const finalRisk: number =
    ((iso.tier + state.riskTier) / 2) * safeTech.techUsageModifier;

  //apply hueristics to model to check for referred
  let ref: boolean = false;

  hueristics.forEach((fn) => {
